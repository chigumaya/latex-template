...
% これとは別に、hyperxmp パッケージも無効化するか、または環境変数SOURCE_DATE_EPOCHで
% 適当な時刻(epochからの経過秒)を指定する必要がある
...
\ifdefined\PRINT
  \usepackage[gray]{xcolor}  % 印刷版ではグレースケール
  \usepackage{jlreq-trimmarks}% トンボあり
\else
...
\usepackage{tablefootnote}   % 表内で脚注
...
\usepackage{eso-pic}         % 全ページに背景描画（隠しノンブル用）
\usepackage{pageslts}        % 通しページ番号(隠しノンブル用だけど電子版でも必要)
...
% デフォの\sffamilyのフォントが日本語とマッチしてないので変更
\setsansfont{TeX Gyre Heros}
% ttfamily のデフォは 0/O、1/l/I の区別がつきづらい
...
\DeclareTColorBox{column}{m}{
  ...
  underlay middle={ % 分割された途中のパーツ(使わない)
